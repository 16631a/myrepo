...
int main() {
    ...
    VideoCapture capLeft("/dev/v4l/by-id/usb-e-con_systems_See3CAM_20CUG_2B199904-video-index0");
    VideoCapture capRight("/dev/v4l/by-id/usb-e-con_systems_See3CAM_20CUG_0E0B9504-video-index0");
    ...
    if (!capRight.isOpened()) {
     ...
     capLeft.set(cv::CAP_PROP_CONVERT_RGB, false);
    capLeft.set(cv::CAP_PROP_EXPOSURE,100);
  capRight.set(cv::CAP_PROP_CONVERT_RGB, false);
  capRight.set(cv::CAP_PROP_EXPOSURE,100);
    ...
    while (true) {
        ...
        capRight.read(rightFrame);
         cv::convertScaleAbs(leftFrame,leftFrame, 0.2490234375);
	 cv::convertScaleAbs(rightFrame,rightFrame ,0.2490234375);
